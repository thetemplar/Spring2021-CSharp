...
{
    class MultiplayerGameManager<T>
    {
        internal int getLeagueLevel()
        {
            throw new NotImplementedException();
        }

        internal IEnumerable<int> getPlayerCount()
        {
            throw new NotImplementedException();
        }
    }
    class Game
    {
        ...
        public static bool ENABLE_SEED;
        public static bool ENABLE_GROW;
        public static bool ENABLE_SHADOW;
        public static bool ENABLE_HOLES;
        ...
        public static bool STARTING_TREES_ON_EDGES;

        Board board;
        Dictionary<int, Tree> trees;
        List<CubeCoord> dyingTrees;
        List<int> availableSun;
        List<Seed> sentSeeds;
        Sun sun;
        Dictionary<int, int> shadows;
        List<Cell> cells;
        Random random;
        int round = 0;
        int turn = 0;
        ...
        {
            return board.map.FirstOrDefault(x => x.Value.index == index).Key;
        }

        private void initStartingTrees()
        {

            List<CubeCoord> startingCoords = new List<CubeCoord>();
            ...
            {
                startingCoords = new List<CubeCoord>(board.coords);


                startingCoords.RemoveAll(x => x.x == 0 && x.y == 0 && x.z == 0);
            }
...
private bool playerCanSeedTo(Cell targetCell, Player player)
{
...
private void doSeed(Player player, Action action) throws GameException
{
    ...
    private bool aTreeIsOn(Cell cell)
{
